        ...
        try {
            ...
            if ($user && password_verify($password, $user['password'])) {
                ...
                if ($remember) {
                    ...
                    // حفظ التوكن في قاعدة البيانات (إضافة عمود remember_token إذا لم يكن موجود)
                    try {
                        $stmt_token = $conn->prepare("UPDATE users SET remember_token = :token WHERE id = :id");
                ...
                try {
                    $stmt_update = $conn->prepare("UPDATE users SET last_login = NOW() WHERE id = :id");
...
// التحقق من Remember Me عند تحميل الصفحة
